...
#GFXBUILD	:=	$(ROMFS)/gfx
APP_AUTHOR	:=	G lander
APP_DESCRIPTION :=      A tile-based puzzle game.
ICON		:=	app/icon.png
...
CFLAGS	+=	$(INCLUDE) -D__3DS__ -D_GNU_SOURCE=1 -DROOTDIR="\"romfs:/\"" -DSAVEDIR="\"save:/\""
...
LIBS	:= -lSDL -lstdc++ -lm -lcitro2d -lcitro3d -lctru 
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(TARGET).elf $(TARGET).3dsx $(TARGET).cia app/*.bin 
	@rm -fr $(OUTDIR)
...
$(OUTPUT).cia	:	$(OUTPUT).elf
	$(BANNERTOOL) makebanner -i "../banner.png" -a "../bannerAudio.wav" -o "../build/banner.bin"

	$(BANNERTOOL) makesmdh -i "../icon.png" -s "$(TARGET)" -l "$(APP_DESCRIPTION)" -p "$(APP_AUTHOR)" -o "../build/icon.bin"
	$(MAKEROM) -f cia -target t -exefslogo -o "./$(TARGET).cia" -elf "../$(TARGET).elf" -rsf "../build-cia.rsf" -banner "../build/banner.bin" -icon "../build/icon.bin" -logo "../icon.png" -DAPP_ROMFS="../$(ROMFS)" -major $(VERSION_MAJOR) -minor $(VERSION_MINOR) -micro $(VERSION_MICRO) -DAPP_VERSION_MAJOR="$(VERSION_MAJOR)"
#---------------------------------------------------------------------------------
