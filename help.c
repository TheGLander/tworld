...
int gameplayhelp(void)
{
	...
	ret = helptilescreen("FLOORS", ARRAY(help_floors), +1) && helptilescreen("WALLS", ARRAY(help_walls), +1) && helptilescreen("OBJECTS", ARRAY(help_objects), +1) && helptilescreen("TOOLS", ARRAY(help_tools), +1) && helptilescreen("MONSTERS", ARRAY(help_monsters), 0);
...
void onlinemainhelp(int topic)
{
	...
	while (displaylist("HELP", &table, &n, LIST_HELP, scrollinputcallback))
	{
		...
		case 1:
			...
			break;
		case 2:
			(void)(helptilescreen("FLOORS", ARRAY(help_floors), +1) && helptilescreen("WALLS", ARRAY(help_walls), +1) && helptilescreen("OBJECTS", ARRAY(help_objects), +1) && helptilescreen("TOOLS", ARRAY(help_tools), +1) && helptilescreen("MONSTERS", ARRAY(help_monsters), 0));
			break;
		case 3:
