 ...
 */
static FILE *FOPEN_with_x(char const *name, char const *mode)
{
	FILE *file = NULL;
	if (!strcmp(mode, "wx"))
	{
						 ...
						 char const *msg)
{
	...
	errno = 0;
	file->fp = FOPEN_with_x(name, mode);
	if (file->fp)
		return TRUE;
...
{
	errno = 0;
	if (fputc(val16 & 0xFF, file->fp) != EOF && fputc((val16 >> 8) & 0xFF, file->fp) != EOF)
		return TRUE;
	return fileerr(file, msg);
}
...
{
	errno = 0;
	if (fputc(val32 & 0xFF, file->fp) != EOF && fputc((val32 >> 8) & 0xFF, file->fp) != EOF && fputc((val32 >> 16) & 0xFF, file->fp) != EOF && fputc((val32 >> 24) & 0xFF, file->fp) != EOF)
		return TRUE;
	return fileerr(file, msg);
}
